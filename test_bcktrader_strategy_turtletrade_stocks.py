#tuttletradingStrategy  -------------------------------------------------optionlike
...
class TurtleTradingStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.position.size > 0:
            # 多单加仓:价格上涨了买入价的0.5的ATR且加仓次数少于等于4次
        ...
        # 如果当前持有空单
            ...
            if self.datas[0].close < self.last_price - 0.5*self.ATR[0] and self.buy_count <= 4:
                print('elf.datas[0].close < self.last_price - 0.5*self.ATR[0] and self.buy_count <= 4:')
                # 计算建仓单位：self.ATR*期货合约乘数300*保证金比例0.1
                self.buy_unit = max((self.broker.getvalue()*0.005)/(self.ATR*300*0.1),1)
                self.buy_unit = int(self.buy_unit)
        ...
        # 如果没有持仓，等待入场时机
            ...
            if self.CrossoverH > 0 & self.buy_count == 0:
                print('if self.CrossoverH > 0 & self.buy_count == 0:')
                # 计算建仓单位：self.ATR*期货合约乘数300*保证金比例0.1
                self.buy_unit = max((self.broker.getvalue()*0.005)/(self.ATR*300*0.1), 1)
                self.buy_unit = int(self.buy_unit)
                self.order = self.buy(size=self.buy_unit)
                ...
                self.buy_count += 1
            #入场: 价格跌破下轨线且空仓时，做空
            elif self.CrossoverL < 0 & self.buy_count == 0:
                print(self.CrossoverL < 0 & self.buy_count == 0)
                # 计算建仓单位：self.ATR*期货合约乘数300*保证金比例0.1
                self.buy_unit = max((self.broker.getvalue()*0.005)/(self.ATR*300*0.1), 1)
                self.buy_unit = int(self.buy_unit)
                self.order = self.sell(size=self.buy_unit)
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Submitted, order.Accepted]:
            self.log('ref:%.0f, name: %s, Order: %s' %(order.ref, order.data._name, order_status[order.status]))
            return
